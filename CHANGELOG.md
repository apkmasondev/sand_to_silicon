...
- **Harmonizacja palety barw & optymalizacja mobile**: Zastąpiono dawny niebieski akcent w `ProgressRail` ciepłym, złoto-miedzianym gradientem z poświatą (`#F5D799` → `#D7A75A` → `#B88432`). Usunięto nieużywane zmienne z `globals.css`, wyłączono natywny niebieski prostokąt tap-highlight (`-webkit-tap-highlight-color: transparent`) oraz wyjęto sztywne rozmiary ikony SVG (`16x16px`) z reguły `@media (hover: hover)`, co naprawiło wielki rozmiar ikony i przycisku na telefonach.
